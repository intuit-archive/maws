...
class CommandParser
  ...
  def initialize(profiles_path, roles_path, commands_path)
    ...
    if File.exists?(@selected_profile_path) && File.exists?(@selected_command_path)
      # parse command options if command available
      load_profile_config
      ...
      # print usage information and error message
      process_generic_help_options
    ...
    p ARGV.length
    process_generic_help if ARGV.include?("-h") && (ARGV.length < 3)
  end
