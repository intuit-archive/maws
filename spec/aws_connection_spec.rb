...
describe 'AwsConnection' do
  before do
    @keyid, @key = aws_test_key
    @ac = AwsConnection.new(@keyid, @key, mash({:region => 'us-west-1', :logger => $right_aws_logger}))
  end
  ...
  describe "for EC2" do
    ...
    it "the interface uses the region specified in options" do
      c = AwsConnection.new(@keyid, @key, mash({:region => 'ap-northeast-1', :logger => $right_aws_logger}))
